...
import { listGlassTypes, createGlassType, updateGlassType, deleteGlassType } from '@/server/functions/admin'
...
const columnHelper = createColumnHelper<GlassType>()
...
function GlassTypesPage() {
  const { glassTypes } = Route.useLoaderData()
  const navigate = useNavigate()
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<GlassType | null>(null)
  ...
  const handleEdit = (item: GlassType) => {
    setEditingItem(item)
    setError('')
    setIsDialogOpen(true)
  }

  const handleDelete = async (item: GlassType) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500">Manage glass options like clear, tinted, Low-E</p>
        </div>
      </div>

      <DataTable
        title="All Glass Types"
        description="Glass options displayed during configuration"
        data={glassTypes || []}
        columns={columns}
        onAdd={handleAdd}
        onEdit={handleEdit}
        onDelete={handleDelete}
        searchPlaceholder="Search glass types..."
      />

      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent onClose={() => setIsDialogOpen(false)}>
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Edit Glass Type' : 'Add Glass Type'}</DialogTitle>
          </DialogHeader>
            ...
            <DialogBody className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="/images/glass/clear.png"
                />
              </div>
            </DialogBody>
            <DialogFooter>
